...
namespace Core
{
	...
	template <typename T> Matrix44<T>& Matrix44<T>::operator+=( const Matrix44<T>& matrix )
	{
		for( int i = 0; i < 16; ++i )
	...
	template <typename T> Matrix44<T>& Matrix44<T>::operator*=( const Matrix44<T>& other )
	{
	...
	template <typename T> Matrix44<T>& Matrix44<T>::operator=( const Matrix44<T>& aMatrix )
	{
	...
	template <typename T> Vector4<T> Matrix44<T>::GetColumn( int index ) const
	{
	...
	template <typename T> Matrix44<T> Matrix44<T>::Identity()
	{
		Matrix44<T> matrix;
	...
	template <typename T> Matrix44<T> Matrix44<T>::Transpose( const Matrix44<T>& mat )
	{
	...
	template <typename T> Vector4<T> Matrix44<T>::GetRadRotations()
	{
	...
	template <typename T> Matrix44<T>::Matrix44( const Matrix44<T>& aMatrix ) { *this = aMatrix; }

	template <typename T> const T Matrix44<T>::GetXRotation() const { return atan2f( m_Matrix[6], m_Matrix[10] ); }

	template <typename T> const T Matrix44<T>::GetYRotation() const
	{
		return atan2f( -m_Matrix[8], sqrtf( ( 2.f / m_Matrix[5] ) + ( 2.f / m_Matrix[6] + 0.00001f ) ) ) + 0.00001f;
	}

	template <typename T> const T Matrix44<T>::GetZRotation() const { return atan2f( m_Matrix[1], m_Matrix[0] ); }

	template <typename T> Matrix44<T> Matrix44<T>::CreateRotateAroundX( const float rad )
	{
	...
	template <typename T> Matrix44<T> Matrix44<T>::CreateRotateAroundY( const float rad )
	{
	...
	template <typename T> Matrix44<T> Matrix44<T>::CreateRotateAroundZ( const float rad )
	{
		const T cosValue = cosf( rad );
		const T sinValue = sinf( rad );
	...
	template <typename T> void Matrix44<T>::RotateAroundPointX( const Vector4<T>& point, float rad )
	{
	...
	template <typename T> void Matrix44<T>::RotateAroundPointY( const Vector4<T>& point, float rad )
	{
		SetPosition( rows[3] - point );
	...
	template <typename T> void Matrix44<T>::RotateAroundPointZ( const Vector4<T>& point, float rad )
	{
		SetPosition( rows[3] - point );
	...
	template <typename T> void Matrix44<T>::SetRotation3dX( const float rad )
	{
	...
	template <typename T> void Matrix44<T>::SetRotation3dY( const float rad )
	{
	...
	template <typename T> void Matrix44<T>::SetRotation3dZ( const float rad )
	{
	...
	template <typename T> void Matrix44<T>::SetTranslation( const T x, const T y, const T z, const T w )
	{
		SetTranslation( { x, y, z, w } );
	}

	template <typename T> void Matrix44<T>::SetTranslation( const Vector4<T>& vector ) { rows[3] = vector; }

	template <typename T> const Vector4<T>& Matrix44<T>::GetTranslation() const { return rows[3]; }

	template <typename T> const Vector4<T> Matrix44<T>::GetRight() const
	{
		return { m_Matrix[0], m_Matrix[1], m_Matrix[2], m_Matrix[3] };
	}

	template <typename T> const Vector4<T> Matrix44<T>::GetUp() const
	{
		return { m_Matrix[4], m_Matrix[5], m_Matrix[6], m_Matrix[7] };
	}

	template <typename T> const Vector4<T> Matrix44<T>::GetForward() const
	{
		return { m_Matrix[8], m_Matrix[9], m_Matrix[10], m_Matrix[11] };
	}

	template <typename T> const Vector4<T> Matrix44<T>::GetScale() const
	{
		return { rows[0].Length(), rows[1].Length(), rows[2].Length(), 1 };
	}

	template <typename T> void Matrix44<T>::SetRight( const Vector4<T>& vector ) { rows[0] = vector; }

	template <typename T> void Matrix44<T>::SetUp( const Vector4<T>& vector ) { rows[1] = vector; }

	template <typename T> void Matrix44<T>::SetForward( const Vector4<T>& vector ) { rows[2] = vector; }

	template <typename T> void Matrix44<T>::SetPosition( const Vector4<T>& vector )
	{
		rows[3].x = vector.x;
		rows[3].y = vector.y;
		rows[3].z = vector.z;
	...
	template <typename T> Matrix44<T> Inverse( const Matrix44<T>& aMatrix )
	{
	...
	template <typename T> Matrix44<T> operator*( const Matrix44<T>& first, const Matrix44<T>& second )
	{
		Matrix44<T> temp( first );
		return temp *= second;
	}

	template <typename T> const Vector4<T> operator*( const Vector4<T>& aVector, const Matrix44<T>& aMatrix )
	{
		Vector4<T> vector;
	...
	template <typename T> Vector4<T>& operator*=( Vector4<T>& v, const Matrix44<T>& m )
	{
	...
	template <typename T> void Matrix44<T>::SetPerspectiveFOV( float fov, float aspect_ratio )
	{
	...
	template <typename T> Matrix44<T> Matrix44<T>::Inverse( const Matrix44<T>& matrix )
	{
	...
	template <class T> Matrix44<T> CreateReflectionMatrixAboutAxis44( Vector3<T> reflectionVector )
	{
	...
	template <typename T> Matrix44<T> InverseReal( const Matrix44<T>& aMatrix )
	{
		T inv[16];
		...
		T det = aMatrix.m_Matrix[0] * inv[0] + aMatrix.m_Matrix[1] * inv[4] + aMatrix.m_Matrix[2] * inv[8] +
				aMatrix.m_Matrix[3] * inv[12];
